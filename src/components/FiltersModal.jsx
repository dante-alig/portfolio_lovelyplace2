 ...
 */
const FiltersModal = ({ isOpen, onClose, locationId, currentFilters, onFiltersUpdate }) => {
  ...
  const handleAddFilter = async () => {
    ...
    try {
      ...
      const updatedFilters = await updateFilters(locationId, "add", [filterString]);
  ...
  const handleRemoveFilter = async (filterString) => {
    try {
      ...
      const updatedFilters = await updateFilters(locationId, "remove", [filterString]);
      // Mise à jour de l'interface utilisateur
      onFiltersUpdate(updatedFilters);
      setError("");
    } catch (error) {
